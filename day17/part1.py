...
ITERATIONS = 125  # determined empirically :>


def main(s: str) -> int:
    x_min, x_max, y_min, y_max = parse_input_data(s)

    def simulate(vx, vy) -> tuple[int, bool]:
        def make_step():
            nonlocal x, y
            nonlocal vx, vy
        ...
        return max_height, is_inside

    vx, vy = 1, 1
    max_height = 0
    for vx in range(ITERATIONS):
        for vy in range(ITERATIONS):
            height, is_inside = simulate(vx=vx, vy=vy)
            if is_inside:
...
def test_main_example_data():
    assert main(INPUT) == 45


def test_main_real_data():
    assert main(parse_input_file()) == 5886
